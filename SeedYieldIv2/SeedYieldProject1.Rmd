...
---
Aim :reads the file with burrs and seeds quantification. Calculates the number of seeds per 1 m2 and seed yield (kg/ha)
...
Read the rawdata file SeedBurrYield.txt
...
Change format of each column and check data.  
...
Calculation of Number of seeds per 1 m2 (Column name = SeedsNo_m2)
...
Graph the Mean values and standard deviation for cultivar and sowing treatments in each Above or below soil scenarios.  
...
Save the dataframe for statistical analysis in another script 

```{r}

write.table(df, "SeedNumberm2_Iv2.txt", row.names=FALSE)

```

Calculate the Seed yield in Kg/ha
...
Write the dataframe with yields for statistics in another script 
```{r}
...
Graph the mean seed yield (kg/ha)  and standard deviation 
...
End of this script. 
More data analysis in SeedYieldProject2 (Stats).
