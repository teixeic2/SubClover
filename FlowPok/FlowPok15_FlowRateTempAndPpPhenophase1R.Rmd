---
title: "Fitting  relationships rate of flowering, mean temperature and Pp targeted at first runner phenophase  "
author: "CT"
date: "30.07.2018"
output:
...
note: build linear relattionships between rate flowering , Temp and Pp at first runner  as per Evans 1992 and Nori 2014, but improved 
...
AIM:
- Estimate build relationship between rate of  flowering (1/days)  in relation to mean temperature and  Pp at first runner phenophase
...
##----------------------------------------- 
#Incorporate the Pp at first Runner  here  
...
#Till here all good ! 
# Now need to bring a data frame with the date of first runner to calculate the Pp at that date 
##Hold here ......Info contained in C:\GitHubRepos\SubCloverPhD\SubClover\RunnersPok
...
getwd()
#create file
df_trifo1 <- read.table("df_DAS_50toRunnerPok.txt",header=TRUE)
head(df_trifo1)
...
df_trifo1<-df_trifo1 %>%
  mutate(SowingDate=as.Date(SowingDate))

#df_trifo1$SowingDate <- ymd(df_trifo1$SowingDate)

df_trifo1 <-df_trifo1 %>%
  dplyr::select(SowTreat: Block, SowingDate, DAS)%>%
  #calculate the date for First trifo 
  mutate(Date50FirstRun=DAS+SowingDate)
  #dplyr::select(-DAS1T)
...
df_trifo1$Date50FirstRun <- ymd(df_trifo1$Date50FirstRun)

df_trifo1$DAS <- as.numeric(df_trifo1$DAS )
...
df_trifo1 <-df_trifo1 %>%
  mutate(Date=Date50FirstRun)%>%
  dplyr::select(-Date50FirstRun)
...
df_1RunnerPpPok <-df_mergePp1Runner %>%
...
#Save as table 
write.table(a, "df_MeanRateFlower50F_TempsAndPp_At1RunnerPhenophase_Pok.txt")
