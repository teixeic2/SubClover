...
-Analysis of trifoliates expansion and runner (V3-V12) for Iversen 2
...
```{r}
setwd("C:\\GitHubRepos\\SubCloverPhD\\SubClover\\PhenophasesAnalysis")
getwd()
#create file
df_trifo <- read.table("ThermalTimevegetativePhenophases2.txt",header=TRUE)
head(df_trifo)
...
#Graph         
  ...
  group_by(Cultivar, SowTreat, TTFromEmerg) %>%
  summarise_each(funs(mean)) %>%
  ggplot(aes(x=TTFromEmerg, y=VegetativePhase)) + 
