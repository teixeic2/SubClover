...
- Calculate the time to reach 5 number of leaves
...
write.table(df_5T,"df_DAS_5_Trifo.txt")
```
...
getwd()
#create file
df_1Trifo <- read.table("RateFirstTrifo.txt",header=TRUE)
str(df_1Trifo)

selected<-df_1Trifo%>%
  dplyr::select(Plot ,DAS1T)
...
subtract DAS5T -DAS1T  


```{r}


df_DaysFrom1T <- merge(df_5T, selected, by="Plot")

head(df_DaysFrom1T)

A<-df_DaysFrom1T %>%
  mutate(Daysto5Tnew=DAS5T-DAS1T)
