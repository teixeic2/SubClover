...
##Determine the date for 1 Trifoliate 
...
df_Temp1 <- merge(dfTTEm,Te, by.x="Date50Emerg",by.y="Date") 

#Hold values to merge later 
...
select(-SumTTday.y, Date, Date50Emerg,GCCode, -SumTTday.x)
  
str(df_merge1.2)

# force formats of dates
df_merge1.2$Date <- round(df_merge1.2$DateTo1TFromEmerg,0)
Te$Date <- round(Te$Date,0)
...
df_merge2 <- df_merge2%>%
  mutate(TTtoFirstTrifo= SumTT1T-SumTTEmerg) 

#give a meaningful name for the df!


df_TT_ToTrifo1_Emerg <-df_merge2
...
df_ANOVA <- df_TT_ToTrifo1_Emerg %>%
  dplyr::select(Cultivar,Block,Pot,TTtoFirstTrifo)
...
file <- df_ANOVA
file$transf_no <- 1*(df_ANOVA$TTtoFirstTrifo)
head(file)
...
##End of this script. 
##(Use similar code to calculate TT to 5T and 9T)
