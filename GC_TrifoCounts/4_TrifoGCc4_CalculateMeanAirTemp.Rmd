...
df1 <- merge(df_MD,df_Temp, by.x="SowingDate",by.y="Date") %>%
  ...
  mutate(Date5Trifo=Date50E)%>%
dplyr::select(-SumTempSo,-SumAir,-DAS50E,-Date50E)
...
df1$Date <- round(df1$Date5Trifo,0)
...
#then repeat same procedure done for the soing date but use the dat for 1 Trifoliate
  ...
  mutate(soilTempAcc_5T = SumTempSo) %>%
  mutate(airTempAcc_5T=SumAir)%>%
  dplyr::select(-SumTempSo, -SumAir) %>%
 
#subtract accumulate temperature das1T from sowing 
  mutate(meanTempSoil_5T = (soilTempAcc_5T-soilTempAcc_sow)/DAS5T ) %>% 
  mutate(meanTempAir_5T = (airTempAcc_5T-airTempAcc_sow)/DAS5T )
  
  summary(df_merge)

  ##Use this dataframe to build graph in script 5 
write.table(df_merge, "df_MeanTempFiveTrifo.txt") 
...
df_TT5T <- df_merge%>%
  mutate(TTto5TSoil=soilTempAcc_5T -soilTempAcc_sow)%>%
  mutate(TTto5TAir=airTempAcc_5T -airTempAcc_sow)
...
dGraph <-df_merge %>%
  ...
  dplyr::select(Rate,meanTempSoil_5T) %>%
  summarise_all(funs(mean,sd)) 
