...
- Calculate the time to reach selected  number of leaves : here five   Trifoliates 
...
#  for(j in 1:length(var)){
 #   for(k in 1:length(sow)){
  ...
  # Interpolate in desired number of leaves: 
...
```{r}
#if needed 
#write.table(df_1T,"GC_df_DAS_5_Trifo.txt")
```
...
df_RateTrifo <- df_5T %>%
  ...
  #write.table(df_RateTrifo,"GC_5T_RateTrifo.txt")

```


##Add the number of days for 50% emergence to calculate the days to five trifoliates after emergence (DAE)

##File ready at : GC_DASToEmergence.txt (already calculated the days after emergence for first trifoliate : DAE_1T) but need to calculate for 5 T !!! 
...
# Prepare and Merrge dataframes by plot
...
##Merge 
 ...
 df_merge <- merge(df_5T, df_DAESelec, by="Plot") %>%
   select(-DAS50E)
 ...
 DAETo5T<-df_merge %>%
   mutate(DAE_5T=as.numeric(DAS5T-DAS_ToEmerg))
...
df_emerg_perc3 <- DAETo5T %>%
  dplyr::select(Cultivar,Block,Plot,DAE_5T)
...
```{r}
##Overview  
##need to specify variate :DAE_5T

file <- df_emerg_perc3
file$transf_no <- 1*(df_emerg_perc3$DAE_5T)
head(file)
