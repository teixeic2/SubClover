...
-calculate the percentage of plants with first trifoliate 
-Analysis of days to first trifoliate appearance 
...
```{r loadFile}
setwd("C:\\Users\\EdCarmen\\Documents\\CarmenProjects2016\\GitSubclover\\FirstTrifo")
getwd()

#create file
df_FT <- read.table("DataFirstTrifo.txt",header=TRUE)

# define factors for the ones that "look like" values
df_FT$Plot <- as.factor(df_FT$Plot)
df_FT$Block <- as.factor(df_FT$Block)
...
df_FT %>% filter(Plot ==1) %>% summary()

#Graph         
  df_FT %>%  group_by(Cultivar, SowTreat, DAS) %>%
#  filter(Cultivar =="Monti") %>%
  #dplyr::select(-SowingDate, - ReadingDate) %>%
  summarise_each(funs(mean)) %>%
  ggplot(aes(x=DAS, y=PercFT)) + 
