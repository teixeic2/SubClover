...
##
file$transf_Asin <- asin(sqrt(file$Hard_perc/100))
#mutate(mydata, sqr.x = x^2)


head(file)

x <- c("transf_sqr", "transf_log","transf_acos","transf_not", "transf_sqrx", "transf_Asin" )
...
#file <- df_final %>%
  file %>%
  dplyr::select(Block, Cultivar,SowTreat,Plot,Depth,transf_Asin)
  #filter(SowTreat=="S1")
...
my.anova <- aov(transf_Asin ~ Cultivar*SowTreat*Depth, data = file.subset)
