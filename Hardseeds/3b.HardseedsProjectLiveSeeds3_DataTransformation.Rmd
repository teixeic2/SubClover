...
This script reads the Hardseeds at the end of thermal treatment  
Runs ANOVA on the least percentage of remaining hardseeds at the end of the thermal treatment
...
#create file
df_seed <- read.table("HardSeedsEndTHermalTreat.txt",header=TRUE)
head(df_seed)
# change format 
...
#data transformation
file$transf_sqr <- sqrt(file$HardEnd)
file$transf_log <- log(file$HardEnd)
file$transf_not <- 1*(file$HardEnd/1)
file$transf_acos <- acos(file$HardEnd/100)
file$transf_sqrx <- (file$HardEnd^2)
...
file %>%
  dplyr::select(Block, Cultivar,SowTreat,Plot,Depth,transf_not)
  #filter(SowTreat=="S1")
...
my.anova <- aov(transf_not ~ Cultivar*SowTreat*Depth, data = file.subset)
