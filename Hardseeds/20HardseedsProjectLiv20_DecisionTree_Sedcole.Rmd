...
df_All <- read.table("InfoDataTree.txt",header=TRUE)
head(df_All)
# change format 
...
#transform arcsin z= arcsin(sqrt(percent/100)). 
A <-df_All %>%
  mutate(SDmaxTrans= asin(sqrt(SDmax/100)))
...
fit <- rpart(SDmax~RH,method="anova", data=df_All)
fit <- rpart(SDmaxTrans~RH,method="anova", data=A)
...
# plot tree 
 plot(fit, uniform=TRUE, 
    main="Regression Tree for SDmax")
