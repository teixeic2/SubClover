...
#summary(lm(HardAccum ~ SoilTemp+RH+Temp, df_All))
#summary(lm(HardAccum ~ RainIrrig+RH, df_All))
...
summary(lm(HSmaxtrans ~ RainIrrig+RH+PET+SoilTemp, df_All6))

summary(lm(HSmaxtrans ~ RH, df_All6))
...
cv.unique <- unique(df_All6$Cultivar)
...
summary(lm(slope ~ RainIrrig+RH+PET+SoilTemp, df_All))
summary(lm(slope ~ RainIrrig+PET, df_All))
...
summary(lm(HSBreaktrans ~ RainIrrig, df_All6))
```

Look also Germination vs environment 
...
write.table(df_All5, "AllDataAllGrowth.txt")
...
summary(lm(Germination ~ RainIrrig+PET+Temp, df_All5))



```

Make graph SDmax vs RH 
...
```{r}
  ...
  xlab(expression(RH~"("*"%"*")"))+ 
  #ylab(expression(Maximum~Hardseeds~Percent~"("*"%"*")"))+
  ylab(expression(HSmax~"("*"%"*")"))+
  #ylab(expression(SDbreak~"( % /"^"o"*"Cd)"))+
  theme(text = element_text(size=22))+
  scale_x_continuous(limits = c(76,81),breaks=c(76,77,78,79,80,81,82))+
  ...
  ggsave("plot_Prjt8_Regression_SDMaxVsRH_AllGrowth.tiff", width=12, height=6, dpi=400)
...
str(df_All6)
  ...
  mutate (SowTreat= factor(SowTreat, levels=c("S1", "S2", "S3", "S4", "S6","S7"),labels=c("June", "July", "September", "November", "February","March")))%>%
  #ggplot(aes(x=RainIrrig, y= slope))  +
  ggplot(aes(x=RainIrrig, y= HSBreaktrans ))  +
  geom_point(colour= "black", alpha=0.2) +
  geom_point(aes(shape=SowTreat),size=6)+
  #geom_point(aes(shape=SowTreat),size=4)+
   geom_smooth(method='lm',formula=y~x, colour="grey",alpha=0.1)+
  ...
  theme(text = element_text(size=22))+
   scale_x_continuous(limits = c(250,900),breaks=c(200,300,400,500, 600, 700,800))+
  ...
  #ggsave("plot_Prjt8_Regression_SDbreakVsRain_AllGrowth.tiff", width=12, height=6, dpi=400)
...
```{r}
  ...
  #geom_point(aes(shape=SowTreat),size=4)+
   geom_smooth(method='lm',formula=y~x, colour="grey",alpha=0.1)+
  theme_bw() +
  xlab(expression(Evapotranspiration~"("*"mm"*")"))+ 
  #ylab(expression(Maximum~Hardseeds~Percent~"("*"%"*")"))+
  #ylab(expression(SDmax~"("*"%"*")"))+
  ylab(expression(SDbreak~"( % /"^"o"*"Cd)"))+
  theme(text = element_text(size=22))+
   scale_x_continuous(limits = c(250,700),breaks=c(200,300,400,500, 600, 700,800))+
  scale_y_continuous(limits = c(0,0.03),breaks=c(0,0.005,0.01,0.015,0.02,0.025,0.03))+
  #facet_wrap(~Cultivar)+
  #remove grid lines 
   theme(
  ...
  ggsave("plot_Prjt8_Regression_SDbreakVsPET_AllGrowth.tiff", width=12, height=6, dpi=400)
