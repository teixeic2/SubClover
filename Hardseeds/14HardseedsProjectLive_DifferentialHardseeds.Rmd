...
av_break<-median(Average$RateDecreasePerTTunit_mean) #= 0.013 # median = 0.12
 ...
 # ggplot(aes(x=RateDecreasePerTTunit_mean, y=HardPerc_Initial14_mean, colour=SowTreat, shape=SowTreat)) +
  #scale_color_manual(values = c("Above" = "black", "Below" = "grey"))+
  #scale_shape(solid = FALSE)+
 ...
 #facet_wrap(~Cultivar,ncol=3) +
  ...
  theme(axis.text.y=element_text(angle = 0, hjust = 0, size= 24))+
  ...
  annotate("text", x = 0.005, y = 110, label = "Hard",size=8)+
  annotate("text", x = 0.025, y = 15, label = "Soft",size=8 )+
  annotate("text", x = 0.005, y = 15, label = "Medium",size=8)+
  annotate("text", x = 0.025, y = 110, label = "Medium",size=8 )+
...
ggsave("plots_Prjt14_HSmaxPlottedHSbreak_Quadrant.tiff", width=16, height=16, dpi=500)
