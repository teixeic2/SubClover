...
# convert to date format
df_Photo <- df_Photo %>%
mutate(DateMeasure=dmy(DateMeasure))
str(df_Photo)

#merge two data frames by ID
df_flower <- merge(df_flower,df_Photo,by="DateMeasure")
head(df_flower)
summary(df_flower )
```
...
#Interpolation date output in strange format 
tryCatch(
  date_50p_flow <- approx(w$avP,w$DateMeasure, xout = 50)$y,
  error = function(e) 
  {
  date_50p_flow <- NA
  }
