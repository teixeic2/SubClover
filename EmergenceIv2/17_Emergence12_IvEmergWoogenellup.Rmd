...
-get dataframe from Iversen Emergence and mean soil Temp for Woogenellup between 5 and 15.0 degrees 
...
Subset cultivars Woog between temperature 5 to max 15 
...
df_W1 <-df_W %>%
  subset (meanTemp_Em>5 & meanTemp_Em<34.7)


#remove SowingDate 1and 8  from both locations 
df_W2 <- df_W1 %>%
   subset (SowTreat=="S1"| SowTreat=="S2"|SowTreat=="S3"|  SowTreat=="S5")
...
summary(lm(InvDAS50E ~ meanTemp_Em, df_W2))
...
##confint(lm(y ~ x, df))
confint(lm(InvDAS50E ~ meanTemp_Em, df_W2))
...
Tb for Wooge = - interc / slope = -(-0.0407811.02)/  0.018820 = 2.16 (?0.01)
