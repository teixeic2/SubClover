...
- merge the two dataframes to calculate the thermal time for Woogenellup using the emergence Tb and cardinal temps. 
...
#---------------------------------------------------------------------------------------------------------------
and Topt , tmax as per germination data

Table XX Mean base (Tb), optimum (Topt) and maximum (Tmax) temperatures (oC) and sub (Ttsub) and supra (Ttsup ) optimal thermal time requirements for 50 % germination of sub clover seeds 'Antas' , 'Denmark', 'Monti' 'Narrikup'.
Cultivar	Tb 	Topt 	Tmax	Ttsub	Ttsup	Excluded temperature
		(oC)		     (oCd)		                    (oC)
'Antas'	 0.05	21.7	34.6	34.48	19.23	-
'Denmark'	-0.10	19.5	34.4	23.81	21.74	-
'Monti'	-0.02	13.5	28.3	34.03	42.58	35
'Narrikup'	-0.07	16.7	31.1	28.48	28.10	35
Max. SE 	 0.03	2.4	6.2		

For Leura use data from Monks 2009 who found Tb=0.1, Topt=20, Tmax=31.For Woogenellup use Tb=1.3; Topt 26 and Tmax 35 (Monks et al 2009; Moot et al 2000 Tb= 0 for Woog. )
# -----------------------------------------------------------------------------------------------------------

For Woogenellup  : 
```{r}
#Create Thermal time cardinal temperatures see data above- TT = Topt - Tb ... 26 - (2.2)= 23.8 ! 

#For cardinal temperatures  
#tt_card <- data.frame(temp=c(2.2,26,26,35),TT=c(0,23.8,23.8,0))

#for TT at Tb=0 
tt_card <- data.frame(temp=c(0,26,26,35),TT=c(0,26,26,0))
tt_card %>%
...
xlab(expression(Temperature~"("^"o"*"C)"))+
  ylab(expression(Thermal~Time~Woogenellup~"("^"o"*"Cd)"))+
...
```{r}
df1 <- merge(df_MD,df_WoogenellupTT, by.x="SowingDate",by.y="Date") %>%
...
df_WoogenellupTT$Date <- round(df_WoogenellupTT$Date,0)


  df_merge <- merge(df1, df_WoogenellupTT, by="Date") %>%
