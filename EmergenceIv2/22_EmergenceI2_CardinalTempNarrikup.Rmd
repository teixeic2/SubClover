...
- merge the two dataframes to calculate the thermal time for Narrikup using the emergence Tb and cardinal temps. 
...
For Leura use data from Monks 2009 who found Tb=0.1, Topt=20, Tmax=31.For Woogenellup use Tb=1.3; Topt 26 and Tmax 35 (Monks et al 2009; Moot et al 200 Tb= 0 for Woog. )
# -----------------------------------------------------------------------------------------------------------
...
#tt_card <- data.frame(temp=c(1,16.7,16.7,31.1),TT=c(0,15.7,15.7,0))

#for TT at Tb=0 
tt_card <- data.frame(temp=c(0,16.7,16.7,31.1),TT=c(0,16.7,16.7,0))
tt_card %>%
...
xlab(expression(Temperature~"("^"o"*"C)"))+
  ylab(expression(Thermal~Time~Narrikup~"("^"o"*"Cd)"))+
...
```{r}
df1 <- merge(df_MD,df_NarrikupTT, by.x="SowingDate",by.y="Date") %>%
...
df_NarrikupTT$Date <- round(df_NarrikupTT$Date,0)


  df_merge <- merge(df1, df_NarrikupTT, by="Date") %>%
