...
- merge the two dataframes to calculate the thermal time for Leura using the emergence Tb and cardinal temps. 
...
For Leura use data from Monks 2009 who found Tb=0.1, Topt=20, Tmax=31.
# -----------------------------------------------------------------------------------------------------------

For Leura  : 
```{r}
#Create Thermal time cardinal temperatures see data above- TT = Topt - Tb ... 20 - (-1.3)= 21.3 ! 

#For cardinal temperatures 
#tt_card <- data.frame(temp=c(-1.3,20,20,31),TT=c(0,21.3,21.3,0))
#for Tb=0
tt_card <- data.frame(temp=c(0,20,20,31),TT=c(0,20,20,0))
tt_card %>%
...
xlab(expression(Temperature~"("^"o"*"C)"))+
  ylab(expression(Thermal~Time~Leura~"("^"o"*"Cd)"))+
...
```{r}
df1 <- merge(df_MD,df_LeuraTT, by.x="SowingDate",by.y="Date") %>%
...
df_LeuraTT$Date <- round(df_LeuraTT$Date,0)


  df_merge <- merge(df1, df_LeuraTT, by="Date") %>%
