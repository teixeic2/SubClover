...
-graph y=Days to 50% emergence ; x= mean soil Temp between 15 and the max observed Temp depending on cultivar!!!  
-graph y= 1/days to 50% emergence ; x= mean soil Temp subset  
-considered only S2 to S7 dataset 
- temperature range here > 15 - 21 
...
library (gridExtra)
```
Read file Subset down means the rate is going down with increase in temperature (Temp > 15)
...
#create file
df_G <- read.table("Subset15_down.txt",header=TRUE)
...
Calculate and include 1/DAS50E 
...
here get the equations for  temperatures  
