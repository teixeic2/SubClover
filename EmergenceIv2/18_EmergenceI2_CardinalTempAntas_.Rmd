...
- merge the two dataframes to calculate the thermal time for Antas using the emergence Tb and cardinal temps. 
...
#tt_card <- data.frame(temp=c(-1.85,21.7,21.7,34.6),TT=c(0,23.5,23.5,0))
#for Tb=0
tt_card <- data.frame(temp=c(0,21.7,21.7,34.6),TT=c(0,21.7,21.7,0))
tt_card %>%
...
```{r}
df1 <- merge(df_MD,df_AntasTT, by.x="SowingDate",by.y="Date") %>%
