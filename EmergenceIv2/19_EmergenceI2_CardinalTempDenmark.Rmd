...
- merge the two dataframes to calculate the thermal time for Denmark using the emergence Tb and cardinal temps. 
...
For Denmark  : 
```{r}
#Create Thermal time cardinal temperatures see data above- TT = Topt - Tb ... 19.5 - (1)= 18.5 ! 

#For cardinal temperatures 
tt_card <- data.frame(temp=c(1,19.5,19.5,34.4),TT=c(0,18.5,18.5,0))
#for Tb=0
#tt_card <- data.frame(temp=c(0,19.5,19.5,34.4),TT=c(0,19.5,19.5,0))
tt_card %>%
...
xlab(expression(Temperature~"("^"o"*"C)"))+
  ylab(expression(Thermal~Time~Denmark~"("^"o"*"Cd)"))+
...
```{r}
df1 <- merge(df_MD,df_DenmarkTT, by.x="SowingDate",by.y="Date") %>%
...
df_DenmarkTT$Date <- round(df_DenmarkTT$Date,0)


  df_merge <- merge(df1, df_DenmarkTT, by="Date") %>%
