...
df_tt_50p_emerg %>%
 ...
 # filter( SowTreat != "S6" & SowTreat != "S7")%>%
  ggplot(aes(x=SowingDate,y=DAS50E, shape=Cultivar, colour=Cultivar))+
  geom_point() +
  geom_line() +
  theme_bw() +
  geom_line()
...
```{r}
df_tt_50p_emerg %>%
  group_by(SowTreat, Cultivar, SowingDate, TT50E) %>%
  summarise_each(funs(mean)) %>%
 # filter( SowTreat != "S6" & SowTreat != "S7")%>%
  ggplot(aes(x=SowingDate,y=TT50E, shape=Cultivar, colour=Cultivar))+
  geom_point() +
  geom_line() +
  theme_bw() +
  geom_line()
