...
info <- read.table("BreakBaseLineReps.txt",header=TRUE)
```


```{r}
df_sp <- info %>%
  mutate(Rep==factor(Rep),IncubationDays=factor(IncubationDays), Seed=factor(Seed)) %>%
  mutate(ID=as.factor(paste0(Cultivar,"_",IncubationDays))) %>%
  dplyr::select(-Seed,-IncubationDays,-Rep,-RowNo,-Cultivar,-Hardness) %>%
  group_by(ID) %>%
  summarise_all(funs(mean)) %>%
  ungroup() 

#df_sp.active <- df_sp[,1:20]

df_sp.active <- df_sp[, colSums(df_sp != 0) > 0]

#df_sp.active<-as.data.frame(df_sp.active)
...
```{r}
rownames(df_sp.active)


```

```{r}
row.names(df_sp.active) <- as.character(df_sp.active$ID)
df_sp.active$ID<-NULL
row.names(df_sp.active)
```
...
# df_sp.active <- df_spA[,1:1738, 1:25]
# 
# 
# 
