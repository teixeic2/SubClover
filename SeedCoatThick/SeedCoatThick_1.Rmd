...
---
Purpose : 
calculations of mean seed coat thickness for plots in Iv2 - cultivars antas, narrikup and monti 
 
ANOVA for the variate : mean seed coat thickness (in microns )
...
setwd("C:\\GitHubRepos\\SubCloverPhD\\SubClover\\SeedCoatThick")
getwd()
file <- read.table("SeecCoatThick.txt",header=TRUE)
...
file <- file %>%
  mutate(
    ...
    MicronsThick= as.numeric(MicronsThick)
  )
...
No need to graph do a table only (few numbers )
...
# theme(axis.text.x=element_text(angle = 0, hjust=0.95,vjust=0.2,size = 14))
# 
# head(df_score)
```
...
my.anova <- aov(MicronsThick ~ Cultivar + Block, 
                data = file.subset)
