---
title: "Seed Breakdown  Thermal treatment- ANOVAS hardseed 84 days "
author: "CT"
date: 16.09.2018
output: 
...
---
This script reads the hardseed dataset of A,M andN cultivars control and 84DAI  (ESA poster dataset) 
...
df_Hard84 <- read.table("HardSeedDataESA.txt",header=TRUE)
#head(df_sp)
...
df_Hard84 <- df_Hard84 %>%
  mutate(Cultivar=factor(Cultivar), Block=factor(Block),DaysIncub=factor(DaysIncub))
...
calculate the SDBreak

```{r}
A <-df_Hard84 %>%
  spread(PercentHard, value, fill = NA, convert = FALSE)
...
##- Now run anova in the decline of hard differential 
...
file.subset <- df_Hard84 

head(file.subset, 50)
summary(file.subset)
  my.anova <- aov(PercentHard ~ Cultivar*DaysIncub, data = file.subset)               
...
#(LSD.test(my.anova, c("Cultivar","DaysIncub"), alpha= 0.05, p.adj="none")) 
