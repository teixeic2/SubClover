...
note: Re analysis data and weather files : sum Vernalization .
---
...
df_CumTTvern2 <- df_CumTTvern %>%
  select(LocationYear, Date, SumVernDay,Pp)
...
df_Ver4 %>%
  mutate(Month=format(as.Date(ymd(SowingDate)),"%b")) %>%
  ggplot(aes(x=reorder(Month,month(SowingDate)), y= VerntoFlower, colour=Treatcultivar)) +
  geom_point(shape=21) +
...
df_tempVern <- df_Ver4%>%
  dplyr::select(AuthorYear, Location, Treatcultivar,VerntoFlower)
