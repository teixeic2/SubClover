...
df_Rea3 %>%
  dplyr::select(LocationYear:Treatcultivar,DaysToFlower,TTtoFlower) %>%
  gather("VarName","VarValue",DaysToFlower:TTtoFlower)  %>%
  mutate(VarName=factor(VarName, level=c("DaysToFlower","TTtoFlower"),labels=c("Days to flower","Thermal-time to flowering"))) %>%
...
#  ggplot(aes(x=reorder(Month,month(SowingDate)), y=DaysToFlower)) +
  ...
  geom_jitter(size=3,alpha=0.7,width = 0.1, aes(colour=reorder(Month,month(SowingDate))))+
  #theme(legend.position="none")+
  labs(x="Trial location", y="Time to flower")+
  theme_bw()+
