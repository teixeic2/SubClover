...
for(i in 1:length(cvGroup)){
  ...
  df_s <- df_Rea6 %>%
    ...
    filter(Variable=="TTFloAdj") %>%
    mutate(TT_target=TTSumSow + t) %>% #define multiple thermal time after sowing for which photoperiod needs to be retrieved 
    rowwise() %>%
  ...
  o <- segmented(out.lm,seg.Z=~Pp_target,psi=list(Pp_target=c(12)),
  ...
  # find stats
      ...
      r2 <- round(summary.segmented(o)$adj.r.squared*100,digits=2)
...
vc_temp <-NULL
 ...
 colnames(df_all) <-c("CvGroup","Ttextra", "r2","slope1","slope2","pVs1","pVs2","b")
...
df_all %>%
   ggplot(aes(x=as.numeric(as.character(Ttextra)), 
...
#graph PPat 1000TTextra 
df_opt <- df_all %>%
group_by(CvGroup) %>%
filter(r2 ==max(as.numeric(as.character(r2))))

tt_extra_opt <- df_opt$Ttextra
...
for(tte in 1:length(tt_extra_opt)) {
   ...
   g <- df_Rea6 %>%
    ...
    filter(Variable == "TTFloAdj") %>%
    mutate(TT_target = TTSumSow + as.numeric(as.character(tt_extra_opt[tte]))) %>% #define multiple thermal time after sowing for which photoperiod needs to be retrieved 
    rowwise() %>%
