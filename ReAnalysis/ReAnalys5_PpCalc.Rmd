...
df_Rea6 %>%
  mutate(Month=format(as.Date(ymd(SowingDate)),"%b")) %>%
  ggplot(aes(x=reorder(Month,month(SowingDate)), y= Value, colour=SensGroup)) +
  geom_point(shape=21) +
  geom_point(aes(shape = factor(SensGroup)))+
  labs(x=" Sowing Month", y="   oCd      Days  ")+
 #geom_smooth(se=FALSE)+
  #theme(legend.position="none")+
  facet_grid(Variable~., scales = "free")
...
# for(tte in 1:length(tt_extra_opt)) {
   ...
   g <- df_Rea6 %>%
    ...
    # stat_smooth(method = "lm", formula = y ~ x + I(x^2), size = 1) + 
     ...
     geom_abline(intercept = ifelse(df_Rea6$SensGroup=="Late",900,700), slope=0,linetype=2)+
...
df_tempVern <- read.table("df_tempVern.txt",header=TRUE)
...
df_ReaVerna <- merge(df_Rea6, df_tempVern,by=c("AuthorYear","Location","Treatcultivar","SowingDate"))

str(df_ReaVerna)

summary(df_ReaVerna)
df_ReaVern <-df_ReaVerna %>%
  mutate(SensGroup=(SensGroup.y)) %>%
 dplyr::select(-SensGroup.x)
   ...
   h <- df_ReaVern %>%
    ...
    mutate(tt_juv_toflower=Value-tt_extra_opt)
     ...
     geom_point(aes(shape=factor(SeasonYear), 
                  ...
                  #  size=reorder(Month, month(SowingDate))
                  )) +
    # stat_smooth(method = "lm", formula = y ~ x + I(x^2), size = 1) + 
     stat_smooth(method = "lm", formula = y ~ poly(x, 2), size = 1)+
     geom_abline(intercept = ifelse(df_Rea6$SensGroup=="Late",900,700), slope=0,linetype=2)+
