...
# for(tte in 1:length(tt_extra_opt)) {
   ...
   g <- df_Rea6 %>%
    ...
    filter(Variable == "TTFloAdj") %>%
     mutate(tt_extra_opt=as.numeric(ifelse(SensGroup=="High",700,700)))%>%
    ...
    #ggplot(aes(x=Pp_target,y=Value)) +
     ggplot(aes(x=Pp_target,y=tt_juv_toflower)) +
    geom_point() + 
    # stat_smooth(method = "lm", formula = y ~ x + I(x^2), size = 1) + 
     stat_smooth(method = "lm", formula = y ~ poly(x, 2), size = 1)+
     geom_abline(intercept = ifelse(df_Rea6$SensGroup=="High",700,700), slope=0,linetype=2)+
