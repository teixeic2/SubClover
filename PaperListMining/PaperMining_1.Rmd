...
Read the list of papers extracted from Liu and Hesson , 2018 

A text mining analysis of proceedings of the New Zealand
Agronomy Society 1971-2017 using Natural Language Processing
...
```{r, echo=FALSE}
setwd("C:\\GitHubRepos\\SubCloverPhD\\SubClover\\PaperListMining")
getwd()
##file <- read.table("CVTrial2017.txt",header=TRUE)

##file <- read.table("allpapers_improved.rds",header=TRUE)

#ListA <- readRDS("allpapers_improved.rds")
## FILE : allpapers_improved.rds

ListB <- readRDS("df.rds")
## FILE : allpapers_improved.rds
...
#summary(List)
 ...
 #write.csv(List, "filename.csv")
 
 write.table(df_ListB, "PapersListB.txt") 


```

Try to convers rds into csv directly 

```{r}

#write.csv(List, file = "PaperList.csv")

#write.table(List, file = "ListPaper.txt", sep = "\t",
#            row.names = TRUE, col.names = NA)
require(reshape2)
List$id <- rownames(List) 
melt(List)


varieties = names(List)
#df <- data.frame(id=varieties,attr(mat, "row.names"), check.rows= FALSE)

df <- data.frame(id=varieties,attr(List, "row.names"))




 write.table(List,file= "PaperList1971_2017.csv", sep = ',')
