...
- Calculate the ti,me to reach 1 number of leaves
...
#  for(j in 1:length(var)){
 #   for(k in 1:length(sow)){
  ...
  # Interpolate ##here choose y_ref = 10 to select 1 trifo phenophase 
  
  y_ref <- 10 # y value where we want to find x by interpolation
...
write.table(df_1T,"df_DAS_1_TrifoPok.txt")
```


# Calculate RAte (1/days to 50%)...Du bist ein genie! 


```{r}

#try 1/DAS50% = Rate_change columns title DAS%0E to DAS1T

df_RateTrifo_1T <- df_1T %>%
  mutate(Rate=1/DAS1T)%>%
  mutate(DAS1T=DAS50E)

  summary(df_RateTrifo_1T)
  
  write.table(df_RateTrifo_1T,"Rate_1T_TrifoPok.txt")

```



Graph rate and SowTreat : note for Sowind dates August and October not all cultivars had the 1 trifoliate  
