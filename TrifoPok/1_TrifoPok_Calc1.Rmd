...
getwd()
#create file
df_trifo <- read.table("TrifoPok.txt",header=TRUE)
head(df_trifo)
...
Graph the results : DAS and number of trifoliates  complete measurements 
...
Here subset until ~ 6 trifoliates 


```{r}

dfSubset<- df_trifo %>%
  subset (Trifo>0 & Trifo<7)
...
#Graph 
  ...
  dA%>%
  ggplot(aes(x=DAS, y=Trifo_mean)) + 
  geom_point(aes(shape=Cultivar,colour=SowTreat)) +
