...
df_emerg %>%
  ...
  facet_wrap(~SowTreat, scales = "free", ncol=4) +
    labs(y="Number of Seedlings per m2",
         x="Days after sowing")
  ...
  summarise_each(funs(mean)) %>%
  ggplot(aes(x=DAS, y=PlantPop, 
             shape = factor(Cultivar),
             colour=factor(SowTreat))) + 
  geom_point() +
    facet_wrap(~SowTreat, scales = "free", ncol=4)+
    labs(y="Number of Seedlings per m2",
         x="Days after sowing")
...
#Fit a linear function TT vs DAS
f <- fitModel(TT_FromSow~A*DAS+B, data=df_emerg_perc_TT)
coef(f)
plotFun(f(DAS)~DAS,DAS.lim=range(0,70),add=TRUE, col="red")
