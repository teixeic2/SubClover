...
-get dataframe produced in Calc4 (df_MeanTempNineTrifo.txt)
-graph y=Days to 9 trifoliate ; x= mean air Temp
-graph y= 1/days to 9trifoliates ; x= mean air Temp
...
df_G <- read.table("df_MeanTempNineTrifo.txt",header=TRUE)
...
```{r}
df_G$DAS9T <- as.numeric(df_G$DAS9T)
...
Graph DAS9T  and rate against meanTemp at trifoliate appearance   do soil Temp first 
...
fontSize <- 12.0
  ...
  g1 <- df_mean %>%
  ggplot(aes(x= meanTempSoil_9T, y=DAS9T))  +
  ...
  ylab("Days to nive trifoliates  (days)") +
...
# 1/DAS9T
  g2 <- df_mean %>%
  ggplot(aes(x= meanTempSoil_9T, y=Rate, shape=SowTreat))  +
  geom_point(aes(shape= SowTreat), size=4)+
  scale_shape_manual(values = c(0,1,2,3,4,5,6,7,8))+
  theme_bw() +
  xlab(expression(Mean~Soil~temperature~~"("^"o"*"C)")) + 
  ylab(" Nine Trifoliates Rate (1/ Days) ") +
  facet_grid(.~Cultivar, scales="free")+
  theme(text = element_text(size=11))+
 scale_x_continuous(limits = c(0,25))+
...
g <-grid.arrange(g1, g2)
  ...
  ggsave(file="plot_DAS_Rate_9TrifoIversenSoilTemp.png", width=12, height=6, dpi=400,g)
# 
...
Graph DAS9T  and rate against Air meanTemp at trifoliate appearance (Air temp now)
...
fontSize <- 12.0
  ...
  g3 <- df_mean %>%
  ggplot(aes(x= meanTempAir_9T, y=DAS9T))  +
  geom_point(aes(shape= SowTreat), size=4)+
  scale_shape_manual(values = c(0,1,2,3,4,5,6,7,8))+
  #geom_smooth()+
  #stat_smooth(level = 0.9, size = 1, colour="white") + 
  ...
  ylab("Days to nine trifoliates (days)") +
    facet_grid(.~Cultivar, scales="free")+
  theme(text = element_text(size=11))+
  scale_x_continuous(limits = c(0,25))+
  scale_y_continuous(limits = c(0,120))
...
# 1/DAS9T
  g4 <- df_mean %>%
  ggplot(aes(x= meanTempAir_9T, y=Rate, shape=SowTreat))  +
  geom_point(aes(shape= SowTreat), size=4)+
  scale_shape_manual(values = c(0,1,2,3,4,5,6,7,8))+
  theme_bw() +
  xlab(expression(Mean~Air~temperature~~"("^"o"*"C)")) + 
...
g <-grid.arrange(g3, g4)
  ...
  ggsave(file="plot_DAS_Rate_9TrifoIversenAirTemp.png", width=12, height=6, dpi=400,g)
# 
