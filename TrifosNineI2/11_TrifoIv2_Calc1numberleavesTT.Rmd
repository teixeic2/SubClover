...
```{r}
bar_x <- max(df_trifo_mean$TTAE_mean)
bar_y <- max(df_trifo_mean$Trifo_mean)
...
df_trifo_mean %>%
 ...
 # geom_smooth(data=TrifoSubset,method='lm',formula=y~x, colour="grey",alpha=0.1, size=2)+
  geom_smooth(data=p1, method='lm', size=2, fill="grey", colour="grey", alpha=0.2) +
...
theme(text = element_text(size = 24))+
  ...
  #label sizes 
theme(text = element_text(size = 26)) + theme(legend.position="top")
...
phylloRegTable %>%
  ...
  facet_wrap(phylloStage~Cultivar, scales='free')
```
...
phylloRegTable %>%
  ...
  geom_point(aes(colour=Block)) +
  geom_boxplot(alpha=0.2) +
  facet_wrap(phylloStage~SowTreat, scales='free')
```
