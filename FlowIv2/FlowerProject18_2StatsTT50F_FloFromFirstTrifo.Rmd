...
```
This script calculates the means of TTAS50Flo from first Trifoliate event  and perform the statistics 
...
df_work<-df_raw %>%
  ...
  dplyr::rename(PhenoStage=Variable)%>%
...
bring data from Days adn TT to first trifoliat (TTFromFirstTrifoToRunner.txt)


```{r}

df_1Tri<- read.table("TTFromFirstTrifoToRunner.txt",header=TRUE)

str(df_1Tri)

##select column plot and TT
df_1Tri<-df_1Tri%>%
  dplyr::select(Plot,TT1TFromEmerg,DAS1TFromEmerg )

```

```{r}

A<-merge(df_work,df_1Tri,by="Plot") 

##discount the frist trifo to get values after first trifo 

str(A)

B<-A%>%
  mutate(TTFloFromFirstTrifo= TT50FromEmerg -TT1TFromEmerg)%>%
  mutate(DAysFloFromFirstTrifo=DAS50FFromEmerg-DAS1TFromEmerg)
...
write.table(df_work2, "TTDAysMergedFlowerFromFirstTrifo.txt")
```

Create a graph TT to Flo against sowing month 
...
#my.anova <- aov(TTFloFromFirstTrifo ~ SowTreat + Block, data = file.subs)
my.anova <- aov(TTFloFromFirstTrifo ~ SowTreat*Cultivar + Block, data = file.subs)
#my.anova <- aov(TTFloFromFirstTrifo ~ Cultivar + Block, data = file.subs)
#my.anova <- aov(DAysFloFromFirstTrifo ~ SowTreat + Block, data = file.subs)
...
head(file.subset, 50)
   summary(file.subset)
my.anova <- aov(TTFloFromFirstTrifo ~ Cultivar + Block, data = file.subset)
