...
dfGraph1 <-dfGraph1 %>%
  ...
  #ungroup()%>%
  dplyr::select(Cultivar,SowingD,SowTreat, Photo , TT50FromEmerg, Pp_target)%>%
...
dfGraph1 %>%
  ...
  theme_bw()+
    ylim(0,1500) + 
  geom_smooth(stat='lm', fill='blue', alpha=0.2, stat = "identity") +
  geom_point(size=2,alpha=0.5,stat = "identity")
...
df_FP_all_mean <- df_FP %>%
    ...
    dplyr::select(Cultivar,Variable,SowTreat,Photo, TT50FromEmerg, TT_target,Pp_target,SowingD)%>%
...
check<- df_FP_all_mean%>%
  filter(TT_target=="200")%>%
...
#make sowing dates white  
  ...
  #remove grid lines 
    theme(
     ...
     ggsave("plot_PhotoThermalTimeFloweringVsPpTarget_AllCvs.tiff", width=14, height=16, dpi=600)

```


graph as sowing date in the X axis and photothermal time at y axis 
...
```{r, fig.width=15, fig.height=10}
  ...
  ungroup() %>%
    ...
    #labels=c("June", "July", "Sept.", "Nov.", "Dec.", "Feb.", "March", "May"))) %>%
    labels=c("Jun", "Jul", "Se", "No", "De", "Fe", "Mar", "May"))) %>%
  ggplot(aes(x=SowingD,y=Photo))   +
  #geom_text(aes(label=SowTreat),hjust=0, vjust=0,size=5) +
  geom_text(aes(label=SowTreat),hjust=0, vjust=0,size=4) +
  ylab(expression(Mean~PhotoThermal~time~Flowering~"("^"o"*"Cd)")) + 
  xlab(" Sowing Month ") +
  facet_grid(Cultivar~TT_target, scales="free")+
  theme_bw()+
...
Inspect values to write 

```{r}
 Monti <-df_FP_all_mean  %>%
   filter (Cultivar=="Monti")

Leura <-df_FP_all_mean  %>%
