...
dfError<- Graphic%>%
  ...
  mutate(SowingD="24/06/2015",mean=600) %>%
  ungroup()
...
##y=100
  ...
  Graphic %>%
  mutate(Month=format(as.Date(ymd(SowingD)),"%b"))%>%
  ggplot(aes(x=reorder(Month,month(SowingD)), y= mean))+ 
  geom_point(size=6, aes( shape=Cultivar),stroke=1.5) +
 
  #theme(legend.position="none")+
  geom_smooth(se=FALSE)+
  #facet_grid(Variable~., scales = "free")
 geom_line(aes(c=Cultivar)) +
...
ggsave("plots_Prj8b_TTBud_SowingMonth.tiff", width=12, height=8, dpi=500)
...
#   ungroup()
  ...
  ungroup() %>%
    ...
    labels=c("Jun", "Jul", "Sept", "Nov", "Dec", "Feb", "Mar", "May"))) %>%
  mutate (Cultivar= factor(Cultivar, 
    levels=c("Antas", "Denmark", "Leura", "Monti", "Narrikup", "Woogenellup"),
    labels=c("A", "D", "L", "M", "N", "W"))) %>%
  #
  ...
  geom_text(aes(label=SowTreat),hjust=0, vjust=0,size=8,alpha=0.3) +
  geom_point(aes(shape=SowTreat),size=6,alpha=1,stroke=1.5)+
...
## Needed to install packages for new version 3.6
```{r}
...
ggsave("plot_8b_DaysANDThermalTimeFlo_VsSowingMonth_AllCvsVersion2.tiff", width=14, height=8, dpi=600)
