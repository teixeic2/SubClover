...
df_t <- read.table("Flo50Iv2.txt",header=TRUE)
...
Then bring function for find Pp at inductive phase 
...
Pp_finder_func <- function(tt_target){
...
for(i in 1:length(cvGroup)){
  ...
  df_s <- df_Iv %>%
    ...
    rowwise() %>%
    mutate(Pp_target=Pp_finder_func(TT_target)) # functions gets a charater therefore format change
